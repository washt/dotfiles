...
class CompletionProvider {
    ...
    interpolationCompletions(document, position) {
        ...
        let replaceRange = new vscode.Range(position.translate(0, -filter.length), document.getWordRangeAtPosition(position).end);
        ...
        }).concat(...builtin_functions_1.GetKnownFunctions().map((f) => this.knownFunctionToCompletionItem(f, needInterpolation, replaceRange)));
    }
    provideCompletionItems(document, position) {
        ...
        if (lineTillCurrentPosition.trim().length === 0 || propertyExp.test(lineTillCurrentPosition.trim())) {
            ...
            if (nestedTypes.length > 0 && parentResource.length > 0) {
                let temp = { items: model_1.allProviders[parentType][parentResource].args };
                let fieldArgs = _.cloneDeep(temp).items;
                if (parentType === "resource") {
                    fieldArgs.push(...model_1.terraformConfigAutoComplete.resource);
                }
                fieldArgs.push(...model_1.allProviders[parentType][parentResource].args);
    ...
    lookupForTerraformResource(lineTillCurrentPosition) {
        ...
        if (parts.length === 2 && (parts[0] === "resource" || parts[0] === "data")) {
            ...
            let possibleResources = _.filter(_.keys(model_1.allProviders[parts[0]]), k => {
                if (regex.test(k)) {
                    return true;
                }
            });
